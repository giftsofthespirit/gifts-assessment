(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const c of i.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&o(c)}).observe(document,{childList:!0,subtree:!0});function n(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(s){if(s.ep)return;s.ep=!0;const i=n(s);fetch(s.href,i)}})();const l=document.querySelector("#app"),e={currentPage:"directions",answers:[],questions:[],categories:[]};async function u(){const r=await(await fetch("/src/questions.txt")).text();e.questions=r.split(`
`).filter(Boolean)}async function d(){const t=await fetch("/src/categories.json");e.categories=await t.json()}async function p(){await u(),await d(),a()}function a(){if(e.currentPage==="directions")l.innerHTML=`
